...
function formatEventTime(iso) {
  if (!iso) return '—';
  try {
    // Handle both ISO strings and SQLite-format strings
...
export default function App() {
  ...
  // ── Generic fetcher with abort + timeout ─────────────────────────────────
  ...
  const fetchAllData = useCallback(async () => {
    ...
    try {
      ...
      if (data.isRealData && data.metrics?.commandsCount) {
        // Try to fetch the command list from bot if exposed; for now use count
      }
    } catch (e) {
...
function HomePage({ setRoute, handleDiscordLogin, liveStatus, connectionError, lastSuccessTime, isLoggingIn, recentEvents, featureFlags, serverInfo }) {
  ...
  // Real protection level: count enabled security features
  ...
  // Real recent events (last 3) for the system event stream
...
// FEATURES PAGE — now with real data
...
function DashboardPage({ user, liveStatus, connectionError, lastSuccessTime, serverInfo, featureFlags, recentEvents, recentSanctions }) {
  if (!user) return null;
  return (
